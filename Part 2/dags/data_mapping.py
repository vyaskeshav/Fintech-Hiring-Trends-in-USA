...
merged_banks = pd.read_csv('/root/airflow/dags/merged_data_all_banks.csv')
...
if os.path.exists("/root/airflow/dags/FinalData.csv"):
    os.remove("/root/airflow/dags/FinalData.csv")
    
with open('/root/airflow/dags/FinalData.csv', "w") as csvFile:
...
for index,row in merged_banks.iterrows():
    ...
    with open('/root/airflow/dags/FinalData.csv', "a") as csvFile:
            wr = csv.writer(csvFile, dialect='excel')
            try:
